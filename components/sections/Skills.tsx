...
const secSkills = [
  ...
  { name: 'OWASP Top 10', tag: 'sec' },
  { name: 'Kali Linux', tag: 'sec' },
  { name: 'SQLMap', tag: 'danger' },
  { name: 'Hydra', tag: 'danger' },
]
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {devSkills.map((skill, i) => (
              <motion.div
                key={skill.name}
                className="portfolio-card flex items-center gap-2 px-4 py-2.5 cursor-default"
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {secSkills.map((skill, i) => (
              <motion.div
                key={skill.name}
                className={`portfolio-card ${skill.tag} flex items-center gap-2 px-4 py-2.5 cursor-default`}
                initial={{ opacity: 0, scale: 0.88 }}
                animate={inView ? { opacity: 1, scale: 1 } : {}}
                transition={{ duration: 0.3, delay: i * 0.04 }}
              >
