    ...
    <section id="skills" className="py-20 px-6 max-w-6xl mx-auto" ref={ref}>
      ...
      <div className="flex flex-col gap-8">
        {categories.map((cat, ci) => {
          ...
          return (
            <div key={cat.key}>
              <h3 className="text-xs uppercase tracking-widest font-bold mb-4" style={{ color: '#fa5252' }}>
                {cat.label}
              </h3>
              <div className="flex flex-wrap gap-3">
                {catSkills.map((skill, i) => (
                  <motion.div key={skill.name} className="rn-card flex items-center gap-2 px-4 py-2.5 cursor-default"
                    ...
                    <span className="text-sm font-medium" style={{ color: 'var(--color-fg)' }}>{skill.name}</span>
                  </motion.div>
                ))}
              </div>
            </div>
