          ...
          {experiences.map((exp, i) => (
            <motion.div key={i} className="relative"
              ...
              <div className="absolute -left-7 top-5 w-3 h-3 rounded-full border-2"
                style={{ background: '#fa5252', borderColor: 'var(--bg)' }} />
              <div className="rn-card p-5">
                <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                  <div>
                    <h3 className="font-semibold text-base" style={{ color: 'var(--fg)' }}>{exp.role}</h3>
                ...
                <p className="text-sm leading-relaxed mb-3" style={{ color: 'var(--muted)' }}>{exp.description}</p>
